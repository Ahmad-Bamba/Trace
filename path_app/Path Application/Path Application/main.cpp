...
unsigned get_char_freq(const char* str, size_t size, char ch) {
	unsigned x = 0;
...
int main(int argc, const char** argv) {
	...
	for(fint i = 0; i < trajectory_l; i++) {
		std::cout << "Coords: (" << trajectory[i].x << "," << trajectory[i].y << ")" << "\t" << "Pos: " << trajectory[i].position << "\t" << "Acceleration: " << trajectory[i].acceleration << "\t" << "Jerk: " << trajectory[i].jerk << "\t" << "Heading (angle): " << r2d(trajectory[i].heading) << std::endl;
	}
	...
	for(fint i = 0; i < trajectory_l; i++) {
		std::cout << "Input: (" << left_trajectory[i].velocity << "," << right_trajectory[i].velocity << ")" << std::endl;
	}
	...
	std::cout << "Cleaned assets!" << std::endl;
